...
<form id="pagerForm" method="post" action="/user.do?tag=toQuery">
...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="/user.do?tag=toQuery" onsubmit="return dwzSearch(this, 'dialog');">
	<input type="hidden" name="rel" value="${rel}"/>
	<div class="searchBar">
	...
	<table class="table" layoutH="118" targetType="dialog" width="100%">
		<thead>
			<tr>
				...
				<th orderfield="userName">用户名称</th>
				<th width="80">查找带回</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${PageUtil.dataSet}" var="user">
			<tr>
				<td>${user.loginName}</td>
				<td>${user.userName}</td>
				<td>
					<a class="btnSelect" href="javascript:$.bringBack({userId:'${user.userId}', userName:'${user.userName}'})" title="查找带回">选择</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
